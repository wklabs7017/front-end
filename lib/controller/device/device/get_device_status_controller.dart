...
class GetDeviceStautsController extends GetxController {
  int? id;
  String? accessToken;
  final Dio dio = Dio();
  ...
  Future<List<Device>> fetchDevicesInRange(
  ...
  Future<List<Device>> getDeviceStatus(int id, String accessToken) async {
    try {
      ...
      if (response.statusCode == 200 && response.data != null) {
        ...
        if (response.data is Map<String, dynamic>) {
          deviceAgvStatus.add(Device.fromJson(response.data));
        }
        // If the response data is a List, iterate over it and
        // convert each item to a Device object and add to the list.
        else if (response.data is List) {
          deviceAgvStatus = (response.data as List)
              .map<Device>(
                  (data) => Device.fromJson(data as Map<String, dynamic>))
              .toList();
        }
