...
// 나머지 import 문들...
class CustomSmartRackStatusDataTable extends StatefulWidget {
  final RxList<SmartRack> smartRacks;
  ...
  CustomSmartRackStatusDataTable({
    required this.smartRacks,
    required this.devices,
  });

  @override
  _CustomSmartRackStatusDataTableState createState() =>
      _CustomSmartRackStatusDataTableState();
}

class _CustomSmartRackStatusDataTableState
    extends State<CustomSmartRackStatusDataTable> {
  ...
  List<TableRow> _generateRows() {
    List<TableRow> rows = [];
    int loopCount = min(widget.smartRacks.length, widget.devices.length);
    ...
    for (int i = 0; i < loopCount; i++) {
      ...
      rows.add(TableRow(
        ...
        children: <Widget>[
          ...
          paddedCell(
              widget.smartRacks[i].ledMode, 12, Colors.black, i), // 인덱스 전달
          paddedCell(
              widget.smartRacks[i].ledMode, 12, Colors.black, i), // 인덱스 전달
    ...
    if (lastTap == null ||
      ...
      setState(() {
        ...
        patchCobotModeController.modeController.text =
            widget.smartRacks[index].ledMode;
                ...
                TextButton(
                  onPressed: () {
                    ...
                    deleteDeviceController
                        .initializeData(widget.devices[index].id);
                    // 선택된 행 삭제
                    widget.smartRacks.removeAt(index);
